...
export class VLCProcessManager {
    ...
    async startVLC(): Promise<void> {
        ...
        try {
            ...
            console.log(`Starting VLC (${this.config.vlcExePath}) with ${guiText}web interface...`);
            
            const args = [
                "--extraintf", "http",       // Add HTTP interface as extra interface
                "--http-password", this.config.vlcPassword,
                "--http-port", this.config.vlcPort.toString(),
            ...
            const command = new (globalThis as any).Deno.Command(this.config.vlcExePath, {
                args: args,
                stdout: "piped",
                stderr: "piped"
            });
    ...
    private async checkVLCWebInterface(): Promise<void> {
        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                const response = await fetch(`http://127.0.0.1:${this.config.vlcPort}/requests/status.json`, {
                    headers: {
