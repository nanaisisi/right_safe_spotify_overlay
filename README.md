...
1. `config_example.toml` を `config.toml` にコピー
2. Client ID と Client Secret を設定
3. 必要に応じて他の設定を調整

#### 3. トークン自動保存機能

このアプリケーションは Spotify 認証トークンを自動的に保存します：

- **初回認証**: Spotify にログインするとトークンが `spotify_tokens.json` に保存されます
- **自動復元**: サーバー再起動時に保存されたトークンが自動的に読み込まれます
- **有効期限管理**: トークンの有効期限を自動的にチェックし、期限切れの場合は再認証を促します
- **セキュリティ**: トークンファイルは `.gitignore` に登録されており、Git 管理外です

> **💡 利点**: 一度認証すれば、サーバー再起動時に毎回ログインする必要がありません
